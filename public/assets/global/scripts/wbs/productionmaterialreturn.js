...
$(function() {
	...
	$('#issuance_no').select2({
		placeholder: "Select Issuance No.",
		dropdownParent: $('#DetailsModal .modal-content'),
		theme: 'bootstrap',
		width: 'auto',
		allowClear: true,
		ajax: {
			...
			data: function (params) {
				...
				var search = "";
				var po = $('#po').val() || "";
				var q = (params.term == undefined)? "" : params.term;

				if (q != "") {
					search = " AND issuance_no LIKE '%"+params.term+"%'";
				}

				query = "SELECT issuance_no as id, \
							issuance_no as `text` \
						FROM tbl_wbs_sakidashi_issuance \
						WHERE po_no = '" + po + "'" + search;
