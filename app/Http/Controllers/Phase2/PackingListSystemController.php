...
class PackingListSystemController extends Controller
{
    ...
    **/
    ...
    //                             'created_at',
    //                             'updated_at',
    //                             DB::raw("IF ((SELECT description from pmi_common.tbl_mdropdowns where id = port_destination) LIKE '%PHILIPPINES%' OR (SELECT description from pmi_common.tbl_mdropdowns where id = port_destination) LIKE '%N/A%', 'N/A', '') AS port_destination_subStr")
    //                         )->get();
    //         }
    //         else
    //         {
    ...
    private function getPackingList($invoicedate_from,$invoicedate_to,$id='')
    {
        try
        {
            ...
            else
            {
                ...
                {
                    # Get records with the given ID.
                    $prodline = strtoupper(Auth::user()->productline);
                    $packinglist = DB::connection($this->mysql)->table('tbl_packing_list')
                                    ->where('id', $id)
                                    ->select('id',
                                        'control_no',
                                        ...
                                        DB::raw("(SELECT description from pmi_common.tbl_mdropdowns where id = carrier) as carrier_name"),
                                        DB::raw("DATE_FORMAT(date_ship, '%m/%d/%Y') as date_ship"),
                                        DB::raw("(SELECT signatory_img FROM pmi_common.tbl_packinglist_setting WHERE user = preparedby AND prodline = '".$prodline."' AND assign = 'preparedby' LIMIT 1) as signatory_img"),
                                        'port_loading',
                                        'port_destination',
                                        DB::raw("(SELECT description from pmi_common.tbl_mdropdowns where id = port_destination) as port_destination_name"),
                                        'description_of_goods',
    ...
    **/
    public function deletePackingList_old(Request $request_data)
    {
        try
        {
    ...
    public function deletePackingList(Request $request_data)
    {
        try
        {
            ...
            if ($this->checkIfDoneInvoice($selectedid)) {
                ...
                $control_no = DB::connection($this->mysql)->table('tbl_packing_list')
                        ->select('control_no','id')
                        ->whereRaw("id in (". $selectedid .")")
                        ->get();
                foreach ($control_no as $k) {
                    #DELETE PACKING DETAILS
                    $q_1 = DB::connection($this->mysql)->table('tbl_packing_list_details')
                    ->where('packing_id',$k->id)
                    ->delete();
                    #DELETE PACKING LIST
                    $q_2 = DB::connection($this->mysql)->table('tbl_packing_list')
                    ->where('id',$k->id)
                    ->delete();
                    #DELETE PACKING YPICS DETAILS
                    $q_3 = DB::connection($this->mysql)->table('ypics_invoicingdetails')
                    ->where([
                        'packinglist_id' => $k->id,
                        'packinglist_ctrl' => $k->control_no,
                    ])
                    ->delete();
                }
                $message = "Selected Packing List Successfully Deleted.";
    ...
    **/
    ...
    public function exportListToPdfPrint(Request $request_data)
    {
        try
        {
            ...
            $grsweight = (isset($result[0]->grossweight_invoicing)) ? $result[0]->grossweight_invoicing : "";
            $port_destination_subStr = (isset($result[0]->port_destination_subStr)) ? $result[0]->port_destination_subStr : "";
            $signatory_id = (isset($result[0]->signatory_img)) ? $result[0]->signatory_img : "default.png";
            $preparedBy_signatory = '';

            if (File::exists(public_path().'/assets/images/signatory/'.$signatory_id)) {
                $preparedBy_signatory = $signatory_id;
            }else {
                $preparedBy_signatory = 'default.png';
            }
            
            $signatory_img = public_path().'/assets/images/signatory/'.$preparedBy_signatory;
            $data = [
                'copy' => $copy,
                'marginT' => $marginT,
                'marginR' => $marginR,
                ...
                'highlight' => $highlight,
                'preparedBy' => $preparedBy,
                'preparedBy_signatory' => $signatory_img,
                'checkedBy' => $checkedBy,
                'grsweight' => $grsweight,
                'id' => $id,
                'result' => $result,
