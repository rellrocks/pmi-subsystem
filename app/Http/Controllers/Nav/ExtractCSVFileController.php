...
class ExtractCSVFileController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (Auth::user() != null) {
            ...
            $this->traffic = $com->userDBcon(Auth::user()->productline,'traffic');
        ...
        foreach ($plines as $key => $pline) 
        {
           ...
           //  $path =  "smb://192.168.3.196/dataextractionpoint/NAV/CN_YPICS_Data";
            ...
            if (file_exists($path)) {
                ...
                {
                    $this->mssql = $com->userDBcon($pline,'mssql');
                    $this->traffic = $com->userDBcon($pline,'traffic');
                    ...
                    Excel::create($pline.$rawmats.$date, function($excel) use($req)
                    {
                        ...
                        {
                            $sheet->cell('A1', "Basic");
                            $sheet->cell('B1', 'Sales Line');
                            $sheet->cell('A3', "Document Type");
                            $sheet->cell('B3', "Document No.");
                            $sheet->cell('C3', "Line No.");
                            $sheet->cell('D3', "Sell-to Customer No.");
                            $sheet->cell('E3', "Type");
                            $sheet->cell('F3', "No.");
                            $sheet->cell('G3', "Shipment Date");
                            $sheet->cell('H3', "Description");
                            $sheet->cell('I3', "Description 2");
                            $sheet->cell('J3', "Quantity");
                            $sheet->cell('K3', "Qty. to Invoice");
                            $sheet->cell('L3', "Qty. to Ship");
                            $sheet->cell('M3', "Unit Price");
                            $sheet->cell('N3', "Unit Cost (LCY)");
                            $sheet->cell('O3', "Amount");
                            $sheet->cell('P3', "Gen. Bus. Posting Group");
                            $sheet->cell('Q3', "Gen. Prod. Posting Group");
                            $sheet->cell('R3', "VAT Bus. Posting Group");
                            $sheet->cell('S3', "VAT Prod. Posting Group");
                            $sheet->cell('T3', "Currency Code");
                            $sheet->cell('U3', "Dimension Set ID");
                            $sheet->cell('V3', "WHT Business Posting Group");
                            $sheet->cell('W3', "WHT Absorb Base");
                            $sheet->cell('X3', "Customer Order No.");
                            $sheet->cell('Y3', "Appl.-to Item Entry");
                            $sheet->cell('Z3', "Businessunit Code (Dimension)");
                            $sheet->cell('AA3', "Details Code (Dimension)");
                            $sheet->cell('AB3', "Expense sub group Code (Dimension)");
                            $sheet->cell('AC3', "Productline Code (Dimension)");
                            $sheet->cell('AD3', "Realaccountsgroup Code (Dimension)");
                            ...
                            $data = DB::connection($this->traffic)
                                ->select("
                                    SELECT 
                                        MIN(ss.for_bir_no) as bir,
                                        MIN(ss.ship_date) as shipdate,
                                        GROUP_CONCAT(DISTINCT ss.description) as description,
                                        SUM(ss.quantity) as qty,
                                        MIN(ss.unitprice) as unitprice,
                                        SUM(ss.quantity * ss.unitprice) as amount,
                                        ss.po_no as po,
                                        MIN(ss.item_code) as item,
                                        MIN(ss.transaction_no) as no,
                                        MIN(ss.sold_to_id) as soldid,
                                        MAX(y.products) as product
                                    FROM ypics_invoicingdetails as ss
                                    LEFT JOIN ypics_invoicings as y ON ss.packinglist_ctrl = y.packinglist_ctrl
                                    WHERE ss.invoice_date between ? AND ?
                                    GROUP BY ss.po_no
                                ", [$from, $to]);

                            foreach ($data as $key => $md) {
                                $com = DB::connection($this->common)
                                    ->table('tbl_soldto')
                                    ->select('companyname')
                                    ->where('code',$md->soldid)
                                    ->first();

                                $sheet->setHeight($row, 20);
                                $sheet->cell('B'.$row, $md->bir);
                                $sheet->cell('D'.$row, $com->companyname);
                                $sheet->cell('F'.$row, $md->no);
                                $sheet->cell('G'.$row, $md->shipdate);
                                $sheet->cell('H'.$row, $md->description);
                                $sheet->cell('J'.$row, $md->qty);
                                $sheet->cell('K'.$row, $md->qty);
                                $sheet->cell('L'.$row, $md->qty);
                                $sheet->cell('M'.$row, $md->unitprice);
                                $sheet->cell('N'.$row, number_format(0, 2));
                                $sheet->cell('O'.$row, number_format($md->amount, 2));
                                $sheet->cell('X'.$row, "\t" . trim($md->po));
                                $sheet->cell('Y'.$row, $md->item);
                                $sheet->cell('AC'.$row, $md->product);
                                $row++; 
                            }
                            
                            $sheet->setAutoFilter('A3:J3');
                            $sheet->setAutoSize(true);
                            $sheet->setAutoSize(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']);
                        });
                    })->export('csv');
                }
                if(strpos($pertable, 'Sales_header') !== false)
                {

                    
                        $this->mssql = $com->userDBcon($pline,'mssql');
                        $this->traffic = $com->userDBcon($pline,'traffic');
                        $this->common = $com->userDBcon($pline,'common');

                        Excel::create($pline.$salesheader.$date, function($excel) use($req)
                        {
                            ...
                            $excel->sheet('Report', function($sheet) use($req)
                            {
                                ...
                                $sheet->cell('A1', "Basic");
                                $sheet->cell('B1', "Sales Header");
                                $sheet->cell('A3', "Document Type"); // SUBSYSTEM
                                $sheet->cell('B3', "No.");
                                $sheet->cell('C3', "Sell-to Customer No.");
                                $sheet->cell('D3', "Bill-to Customer No."); // YPICS
                                $sheet->cell('E3', "Bill-to Name"); // YPICS
                                $sheet->cell('F3', "Bill-to Name 2"); // YPICS
                                $sheet->cell('G3', "VAT Registration No."); // SUBSYSTEM 
                                $sheet->cell('H3', "Bill-to Address"); // YPICS
                                $sheet->cell('I3', "Bill-to Address 2"); // YPICS
                                $sheet->cell('J3', "Bill-to City");
                                $sheet->cell('K3', "Ship-from Address"); // SUBSYSTEM 
                                $sheet->cell('L3', "Ship-from Address2"); // SUBSYSTEM 
                                $sheet->cell('M3', "Your Reference"); 
                                $sheet->cell('N3', "Order Date"); 
                                $sheet->cell('O3', "Posting Date"); // YPICS
                                $sheet->cell('P3', "Pick-upd Date"); // YPICS
                                $sheet->cell('Q3', "Shipment Date"); // YPICS
                                $sheet->cell('R3', "Posting Description"); // subsystem
                                $sheet->cell('S3', "Payment Terms Code"); // subsystem
                                $sheet->cell('T3', "Salesperson Code");
                                $sheet->cell('U3', "Due Date");
                                $sheet->cell('V3', "Customer Posting Group");
                                $sheet->cell('W3', "Currency Code");
                                $sheet->cell('X3', "Currency Factor");
                                $sheet->cell('Y3', "Shipping No."); // subsystem
                                $sheet->cell('Z3', "Posting No."); // subsystem
                                $sheet->cell('AA3', "Gen. Bus. Posting Group");
                                $sheet->cell('AB3', "Sell-to Customer Name"); // sub
                                $sheet->cell('AC3', "Sell-to Customer Name 2"); // sub
                                $sheet->cell('AD3', "Sell-to Address"); // sub
                                $sheet->cell('AE3', "Sell-to Address 2"); // sub
                                $sheet->cell('AF3', "Sell-to City"); // sub
                                $sheet->cell('AG3', "Document Date"); // sub
                                $sheet->cell('AH3', "External Document No."); // sub
                                $sheet->cell('AI3', "Payment Method Code");
                                $sheet->cell('AJ3', "No. Series");
                                $sheet->cell('AK3', "Posting No. Series"); // sub
                                $sheet->cell('AL3', "VAT Bus. Posting Group");
                                $sheet->cell('AM3', "Assigned User ID");
                                $sheet->cell('AN3', "WHT Business Posting Group");
                                $sheet->cell('AO3', "Remarks");
                                $sheet->cell('AP3', "Entered By"); // sub
                                $sheet->cell('AQ3', "SHIPPED VIA"); // sub
                                $sheet->cell('AR3', "FREIGHT TERMS"); // sub
                                $sheet->cell('AS3', "No. of Packages"); // sub
                                $sheet->cell('AT3', "Gross Weight"); // sub 
                                $sheet->cell('AU3', "Name of Cargo");
                                $sheet->cell('AV3', "Businessunit Code (Dimension)");
                                $sheet->cell('AW3', "Details Code (Dimension)");
                                $sheet->cell('AX3', "Expense sub group Code (Dimension)");
                                $sheet->cell('AY3', "Productline Code (Dimension)");
                                $sheet->cell('AZ3', "Realaccountsgroup Code (Dimension)");
                                $sheet->cell('BA3', "Packing List");
                                $sheet->cell('BB3', "Net Weight");
                                $sheet->cell('BC3', "Note");
                                $sheet->cell('BD3', "Highlights");
                                $sheet->cell('BE3', "Tel");
                                $sheet->cell('BF3', "S/No.");
                            ...
                            $data = DB::connection($this->traffic)
                                        ->select("
                                                SELECT 
                                                ss.for_bir_no as bir,
                                                ss.sold_to_id as soldid,
                                                ss.shipto_address as address,
                                                ss.shippedfrom as `from`, 
                                                ss.country_origin as country,
                                                ss.pickup_date as pcd,
                                                ss.invoice_date as inv,
                                                ss.sailing_on as sailing,
                                                y.invoice_no as posting,
                                                ss.terms_of_payment as top,
                                                ss.awb_no as awb,
                                                ss.shipto_address as shipto,
                                                ss.shipto as city,
                                                ss.prepared_by as user,
                                                ss.via as via,
                                                ss.po_no as po,
                                                ss.freight as freight,
                                                ss.no_of_packaging as packaging,
                                                ss.gross_weight as gweight,
                                                ss.case_marks as marks,
                                                ss.carrier as cargo,
                                                ss.note_hightlight as net,
                                                ss.highlight as highlight,
                                                y.products as product,
                                                y.packinglist_ctrl as ctrl,
                                                p.note as note,
                                                p.tel_no as tel_no,
                                                p.remarks_s_no as s_no
                                            FROM ypics_invoicingdetails as ss
                                            LEFT JOIN ypics_invoicings as y ON ss.packinglist_ctrl = y.packinglist_ctrl
                                            LEFT JOIN tbl_packing_list as p ON ss.packinglist_ctrl = p.control_no
                                            WHERE ss.invoice_date between '".$from."' AND '".$to."' 
                                            ");
                                                    
                                        foreach ($data as $key => $md) {
                                            $com = DB::connection($this->common)
                                                ->table('tbl_soldto')
                                                ->select('companyname', 'vatreg_no','description')
                                                ->where('code',$md->soldid)
                                                ->first();

                                            $description_parts = explode("\n", $com->description);
                                            $company_name = isset($description_parts[0]) ? $description_parts[0] : '';  // First line (company name)
                                            $address_line_1 = isset($description_parts[1]) ? $description_parts[1] : ''; // Second line (address)
                                            $address_line_2 = isset($description_parts[2]) ? $description_parts[2] : '';
                                            $address_line_3 = isset($description_parts[3]) ? $description_parts[3] : '';


                                            preg_match('/TEL:\s*(.+)/i', $com->description, $matches);
                                            $telephone = isset($matches[1]) ? $matches[1] : '';

                                            $from = DB::connection($this->common)
                                                ->table('tbl_soldto')
                                                ->select('description')
                                                ->where('code','=',10021)
                                                ->first();
                                            ...
                                            $frompmi = explode("\n", $from->description);
                                            $company_name = isset($frompmi[0]) ? $frompmi[0] : '';  // First line (company name)
                                            $frompmi1 = isset($frompmi[1]) ? $frompmi[1] : ''; // Second line (address)
                                            $frompmi2 = isset($frompmi[2]) ? $frompmi[2] : '';

                                            $shipto = explode("\n", $md->address);
                                            $shiptoname = isset($shipto[0]) ? $shipto[0] : '';
                                            $shipto2line = isset($shipto[1]) ? $shipto[1] : ''; // 2nd line
                                            $shipto3line = isset($shipto[2]) ? $shipto[2] : ''; // 3rd line

                                            $attn_lines = array_filter($shipto, function($line){
                                                return strpos(trim($line), "ATTN") === 0;
                                            });
                                            ...
                                            $notes = explode(" ", $md->net, 2);
                                            $desc = isset($notes[1]) ? $notes[1] : '';


                                            $sheet->setHeight($row, 20);
                                            $sheet->cell('B'.$row, $md->bir);
                                            $sheet->cell('E'.$row, $com->companyname);
                                            $sheet->cell('F'.$row, $com->vatreg_no);
                                            $sheet->cell('H'.$row, $address_line_1);
                                            $sheet->cell('I'.$row, $address_line_2,$address_line_3);
                                            $sheet->cell('K'.$row, $frompmi1);
                                            $sheet->cell('L'.$row, $frompmi2);
                                            $sheet->cell('M'.$row, $md->country);
                                            $sheet->cell('O'.$row, $md->inv);
                                            $sheet->cell('P'.$row, $md->pcd);
                                            //$sheet->cell('Q'.$row, $md->sailing);
                                            $sheet->cell('Q'.$row, function($cell) use ($md) {
                                                $cell->setValue(Carbon::parse($md->sailing)->format('d-m-y'));
                                            });
                                            $sheet->cell('R'.$row, $md->posting); 
                                            $sheet->cell('S'.$row, $md->top);
                                            $sheet->cell('Y'.$row, $md->awb);
                                            $sheet->cell('Z'.$row, $md->bir);
                                            $sheet->cell('AB'.$row, $shiptoname);
                                            $sheet->cell('AC'.$row, $attn_text);
                                            $sheet->cell('AD'.$row, $shipto2line);
                                            $sheet->cell('AE'.$row, $shipto3line);
                                            $sheet->cell('AF'.$row, $md->city);
                                            $sheet->cell('AG'.$row, $md->inv);
                                            $sheet->cell('AH'.$row, $md->posting);
                                            $sheet->cell('AK'.$row, $md->bir);
                                            $sheet->cell('AH'.$row, $md->posting);
                                            $sheet->cell('AO'.$row, $md->marks);
                                            $sheet->cell('AP'.$row, $md->user);
                                            $sheet->cell('AQ'.$row, $md->via);
                                            $sheet->cell('AR'.$row, $md->freight);
                                            $sheet->cell('AS'.$row, $md->packaging);
                                            $sheet->cell('AT'.$row, $md->gweight);
                                            $sheet->cell('AU'.$row, $md->cargo);
                                            $sheet->cell('AX'.$row, $md->product);
                                            $sheet->cell('BA'.$row, $md->ctrl);
                                            $sheet->cell('BB'.$row, $md->highlight);
                                            $sheet->cell('BC'.$row, $md->note);
                                            //$sheet->cell('BD'.$row, $md->note);
                                            $sheet->cell('BE'.$row, $md->tel_no);
                                            $sheet->cell('BF'.$row, $md->s_no);
                                            $row++; 
                                        }
                        });
                    })->export('csv');
                ...
                $this->mssql = $com->userDBcon($pline,'mssql');
                    Excel::create($pline.$packinglist.$date, function($excel) use($req)
                    {
                        ...
                        $data = DB::connection($this->mssql)
                                            ->select("
                                                    SELECT r.SORDER as PONumber,
                                                    r.CODE as Product_Code, 
                                                    h.NAME as Product_Name,
                                                    r.JITU as Quantity, 
                                                    r.APRICE as Price, 
                                                    r.URIAGEGAKU as Sales_Amount,
                                                    CAST(LEFT(r.IDATE,8) as date) as Entry_Date, 
                                                    CAST(LEFT(r.FDATE,8) as date) as Actual_Ship_Result_Date, 
                                                    r.PACKING_LIST as Packing_List_No, 
                                                    r.INPUTUSER as PreparedBy
                                                    FROM XRACT r 
                ...
                if(strpos($pertable,'Consumption')!== false)
                {
                    ...
                    Excel::create($pline.$BOH.$date, function($excel) use($req)
                    {
                        ...
                        $data = DB::connection($this->mssql)
                                    ->select("SELECT z.CODE as 
                                    Part_Code ,h.NAME as 
                                    Part_Name, z.ZAIKTANA as BOH,
                                    ISNULL(z1.ZAIK,0) + ISNULL(z2.ZAIK,0) as CurrentInventory,
                                    UPPER(h.TANI1) as UOM, 
                                    ISNULL(t.SPRICE,0) as UnitCost , 
                                    i.BUNR as PartType
                                    FROM XZAIK z 
                                    LEFT JOIN XZAIK z1 ON z1.CODE = z.CODE AND z1.HOKAN = 'WHS100'
                                    LEFT JOIN XZAIK z2 ON z2.CODE = z.CODE AND z2.HOKAN = 'WHS102'
