		    			...
		    			<div class="col-xs-12">
		    				<table class="table" style="font-size: 10px">
		    					<tr>
									...
									<td class="align-left">Part Name:</td>
		                            <td>{{ $x->partname }}</td>
		                        ...
		                        <tr>
									...
									<td class="align-left">Part Code</td>
		                            <td>{{ $x->partcode }}</td>

		                            <td class="align-left">Severity of Inspection</td>
		                        ...
		                        <tr>
									...
									<td class="align-left">Supplier</td>
		                            <td>{{ $x->supplier }}</td>
		    		...
		    		<div class="row">
		    			<div class="col-xs-12">
							...
							<?php
			                    ...
			                    $final_supplier = "";
			                    $final_partcode = "";
			                    $pos1 = strpos($x->supplier, $findMe);
			                    $pos2 = strpos($x->partcode, $findMe);

			                    if($pos1 == ""){
			                        $final_supplier = $x->supplier;
			                    }else{
			                        $final_supplier = substr($x->supplier, 0, $pos1 );
			                    }

			                    if($pos2 == ""){
			                        $final_partcode = $x->partcode;
			                    }else{
			                        $final_partcode = substr($x->partcode, 0, $pos2 );
			                    }

								$sql = "SELECT id, fy, partcode, ww, date_ispected, shift, time_ins_from, time_ins_to, submission, app_no, 
												invoice_no, lot_no, lot_qty, sample_size, no_of_defects, judgement, inspector, remarks
								...
								$details = \DB::connection($conn)->select($sql);
		    				...
		    				<table class="table table-striped table-bordered table-condensed" style="font-size:11px">
		    					<thead>
		                            <tr>
										...
										<th>Quantity</th>
										<th>Determination on Lot Acceptability</th>
										<th>Inspector</th>
										<th>Remarks</th>
		                            </tr>
		                        </thead>
									...
									<tr>
										...
										<td>
											<?php
												$sql = "SELECT lot_no 
					   							...
					   							$lot_no = \DB::connection($conn)->select($sql);
												$lot_arr = [];
												// ITERATE LOT NO
										...
										<td>
											<?php 
												...
												$lot_qty = \DB::connection($conn)->select($lot_qtySql);
										...
										<td>
											<?php
												$mode_of_defects = \DB::connection($conn)->table('tbl_mod_iqc_inspection')
																		->select('mod')
																		->where('iqc_id', $qc->id);
												...
												if ($check_count > 0) {
													...
													$mode_of_defects = \DB::connection($conn)->table('tbl_mod_iqc_inspection')
																		->select('mod')
																		->where('invoice_no', $qc->invoice_no)
																		->where('partcode', $qc->partcode)
																		->where('lot_no', $qc->lot_no)
																		->get();
										...
										<td>
											<?php
												$mode_qty = \DB::connection($conn)->table('tbl_mod_iqc_inspection')
																		->select('qty')
																		->where('iqc_id', $qc->id);
												$check_count_qty = $mode_qty->count();
												if ($check_count_qty > 0) {
													foreach($mode_qty->get() as $key => $q){
														echo $q->qty. "<br/>";
													}
												}else{
													$mode_qty = \DB::connection($conn)->table('tbl_mod_iqc_inspection')
